...
\toggletrue{digital}     % PC version - centered margins
%\togglefalse{digital}   % Print version - shifted margins
...
\thesissetup{
    ...
    thanks        = {To Petr Švenda, for guidance, inspiration, and the ability to see the light at the end of the tunnel. To Marek Sýs, for helping me understand both trivial and not-so-trivial concepts of the world of probability and statistics. To an exceptional person, for distant, yet far from pointless support. To all the others, both from the lab and the outside, for ideas, discussions, suggestions, and procrastination that often kept me sane.\\ \\
    Thank you. I could not have made it without you. \\ \vspace{\fill} \\ 
    Computational resources were supplied by the Ministry of Education, Youth and Sports of the Czech Republic under the Projects CESNET (Project No. LM2015042) and CERIT-Scientific Cloud (Project No. LM2015085) provided within the program Projects of Large Research, Development and Innovations Infrastructures. \\ We also acknowledge the support of Czech Science Foundation, the project GA16-08565S.},
...
\iftoggle{digital}{
	...
	\newcommand{\rmar}{2cm} % Printed
	\newcommand{\tmp}{2cm}  % Printed
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
% Overriding default layout.  I like it this way more.
...
\texttt{finished} & The jobs execution ended. & The execution of all related jobs ended. \\ \bottomrule
\end{tabular}
\end{nomar}
...
\item \textbf{Suspect} ($0.001 \leq P(x) < 0.01$) -- The result is considered suspicious. The user should generate more data from the source and test it again. This result is possible to happen by chance even when the tested data is produced by a good source.
...
\end{nomar}
\caption{Histograms of p-values of chosen atomic tests. Only the Min Distance (2d Circle) test produces uniform p-values.}
...
\begin{nomar}
\centering
\includegraphics[width=\textwidth]{figures/submit-experiment-form.png} 
\end{nomar}
\end{figure}
