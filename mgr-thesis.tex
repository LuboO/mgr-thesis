...
When we are evaluating a battery analysis based on number of failed tests, we must know how many tests we expect to fail when the $H_0$ holds true. We can calculate the expected value like so. Let $n$ be the number of tests in a battery. Then we have $n$ independent experiments that will result in either yes (failure of the test) with probability $\alpha$ or no (success of the test) with probability $1-\alpha$. This situation can be modeled by using binomial distribution (cit.) $B(n, \alpha)$. Let $F_{Exp}$ be a random discrete variable expressing number of failed tests in a battery with expected distribution $B(n, \alpha)$. From the definition of the binomial distribution, the expected number of the failed tests is $E(F) = n \times \alpha$.
...
\begin{description}
\item[Raw second-level p-values failures] \hfill \\
...
Whit this second scenario, we tried to mitigate the effect of dependent p-values in our set by grouping them together. We put the p-values that are most likely dependent into single set or group. We assumed that the p-values that are most likely inter-connected are those that are produced by a single statistical test. Therefore, all p-values belonging to a single test were reduced to only one p-value, leaving us with a single result per distinct test in a statistical battery. For the grouping of the p-values we used Šidák's correction (cit.) and the process is shown in Definition~\ref{def:sidak}.

\begin{definition}
\label{def:sidak}
For $n$ grouped p-values $\{p_1,..,p_n\}$ with significance level $\alpha$ and null hypothesis $H_0$ we will calculate the corrected significance level $\alpha_0 = 1 - (1 - \alpha)^{\frac{1}{n}}$. If $\forall i \in [1,..,n], \exists p_i : p_i < \alpha_0$, then $H_0$ is rejected for $\{p_1,..,p_n\}$ and the whole group result is treated as a failure.
\end{definition}

\end{description}

\begin{Huge}
TODO: tell them about observed and expected counts and chi-square comparison
\end{Huge}
