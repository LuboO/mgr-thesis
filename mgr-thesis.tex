...
Let's have a multiset $X=\{1,3,4,3,..,0\}$, in which are stored the counts of failed tests in 1000 executions of Dieharder battery (in the first execution failed one test, in the second execution three tests, etc.); therefore we have $s=1000$. There are $n=110$ uncorrected tests in the Dieharder battery. In Figure~\ref{fig:corr_vs_orig_hist} we can see the observed and expected frequencies of the failure counts for both corrected and uncorrected tests. We calculate the expected frequency for each failure count as follows (Equation~\ref{eq:bin_pmf}, Equation~\ref{eq:exp_freq}); we use zero failure count as an example.

$$
E(f_0^X) = s P(F=0) = 1000 \binom{110}{0} 0.01^0 (1-0.01)^{110-0} \approx 331.03
$$

Based on the expected and observed frequencies in the Figure~\ref{fig:corr_vs_orig_hist} we calculate the $\chi^2$ statistic (Equation~\ref{eq:chi_square_stat}); 

\begin{align*}
\chi^2 &= \sum\limits_{i=1}^{k} \frac{  \left(f_{d_i}^{X} - E \left(f_{d_i}^{X}\right) \right)^2 }{E \left(f_{d_i}^{X}\right)} \\
       &= \frac{(460 - 331.03)^2}{331.03} + \frac{(331 - 367.81)^2}{367.81} + \cdots + \frac{(7 - 5.12)^2}{5.12} \\
       &\approx 85.71
\end{align*}
...
We can compare the $\chi^2$ p-values shown in Figure~\ref{fig:corr_vs_orig_hist}. As the p-values suggest, in the corrected version of failure counting, the observed failure frequency is much closer to the expected distribution than in the original version.
