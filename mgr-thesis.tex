...
\section{Analysis of the uniformity of p-values}
...
If we observe $x$ failures, we can calculate the probability of observing same or more extreme count of failures under the assumption that $F_{Obs}$ follows the distribution $\text{Bin}(n,\alpha)$. The more extreme result is a result more distant from the expected value than the observed one. The probability is calculated as shown in Equation~\ref{eq:prop_statistic}.
