...
We named the interface Randomness Testing Toolkit\footnote{Also refered to as RTT or just the toolkit.} \cite{rtt-github} and described its functionality and implementation details in Chapter~\ref{chap:rtt}. The overview of the software used in RTT is in Chapter~\ref{chap:batteries}. 
...
A type I error (false positive) is the rejection of a true null hypothesis $H_0$ \cite{type-i-ii-error}. On the opposite, a type II error (false negative) is incorrectly holding to the null hypothesis, even when it should have been rejected.
