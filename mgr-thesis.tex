...
%\item Conlusions - limits of EACirc, limits of RTT, differences, Rabbit, Grain are lit af.
%\end{itemize}
...
For each suspicious algorithm with fixed round we generated additional 16000 MiB of data. The additional data shown same patterns in the results as in the original analysis, hinting the presence of a certain bias produced by the algorithm.
...
Rabbit cipher is one of the winners in the eSTREAM competition in the software section. The algorithm has total of 4 rounds. In all of its configurations, Rabbit will consistently fail tests \texttt{sstring\_HammingIndep} and \texttt{sstring\_PeriodsInStrings} in batteries Crush, Rabbit, Alphabit and Block Alphabit. 
...
RC4 is a well-known stream cipher widely used in WEP and TLS up until version 1.3. Various attacks on RC4 were published and in 2015 it was prohibited for use in TLS by RFC 7465 (cit.). In its full version, the RC4 cipher will consistently fail tests \texttt{sknuth\_SimpPoker} and \texttt{sknuth\_Gap} in the Crush battery.

Multiple papers on biases and distinguishing attacks on RC4 were published; notable publications are \textit{A Practical Attack on Broadcast RC4} (cit.), \textit{Analysis of Non-fortuitous Predictive States of the RC4 Keystream Generator} (cit.) and \textit{Statistical Analysis of the Alleged RC4 Keystream Generator} (cit.).

\item[Security margins with respect to the statistical analysis] \hfill \\
Majority of the analysed algorithms has the security margin higher than 0.8. This means that even if we would drastically increase our computational capability and performance of the state-of-the-art statistical tools we still wouldn't be able to detect biases in most of the function outputs. 

The margins are significantly lower only in functions with specific biases detected, namely Rabbit, RC4 and Grain.

\item[Strength of the batteries and EACirc] \hfill \\
All of the batteries implemented in RTT perform better or equally well than EACirc. This can be caused by the fact that EACirc analyses only 100 MB of data per execution and also finishes its execution more quickly. Therefore we have trade-off between the needed time and resources and strength of the tool. In many situations we aren't able to retrieve 8000 MiB (e.g. slow hardware generators in smartcards); in these cases the usage of EACirc for bias detection can be viable.
...
The MD6 hash function was a competitor in the SHA-3 competition but did not advance to the second round of the competition. In 9 and 10 rounds out of 94, the algorithm will consistently fail tests \texttt{smarsa\_MartixRank} and \texttt{sspectral\_Fourier3} in batteries Crush and Rabbit. While this hints us with the presence of a specific bias in the function output it does not break the security of the function; in further rounds the bias is lost and the output appear as truly random.
