...
The web browser interface is intended to accomodate the needs of common users. The main idea is that the user will visit our website and, using the interface, will create his experiment. For basic testing the user is only required to upload the data and the suitable battery configuration will be chosen for him. Alternatively one can also manually choose the desired configuration or create his own configuration from the scratch according to his preferences. The web interface is not intended to be used for creating a big amount of experiments because it would be too time consuming for the user to create all the experiments manually.

Another purpose of the web interface is to present results of the previous analyses to the user and help him interpret those results. 

The web interface is not dependent on or required by the underlying server infrastructure that realizes analysis of the data. 

We split possible the users into three roles: anonymous users, authorized users and administrators of the interface. The anonymous users can browse results of the previous analyses. The anonymous users can be given unique access code that will allow them to access creation of experiments that is otherwise restricted. The authorized users can freely create new experiments. The administrator can add or modify users (with or without administrator privileges), add or modify predefined battery configurations and create and set expiration date on the access codes for acces to experiment creation. 
