...
When analysing a truly random data repeatedly with a statistical test, we will observe that the resulting p-values are uniformly distributed over the interval $[0,1]$. The uniformity is caused by the fact that we cannot predict the output of a TRNG and each possible output is observed with equal probability. Therefore, we also cannot predict the result of the analysis with a statistical test. Since the p-values are uniformly distributed, they will fall outside of the interval $[\alpha, 1)$ with probability equal to $\alpha$; test's $H_0$ will be rejected with probability $\alpha$. When the null hypothesis of a test is rejected, we assess the test as failed.
...
\includegraphics[clip, width=.42\paperwidth]{figures/dieharder-corr.pdf} 
